...
var getPosts = function(url, slug, callback) {
	listCache.get(slug, function(err, posts) {
		if (!err && !isEmpty(posts)) {
			...
			generateHackerNewsPosts(url, slug, callback);
		} else {
			res.status(500).send('Something went wrong while getting the cache value for ' + slug);
		}
	});
};
	...
	request({
		...
		listCache.set(slug, posts, function(err, success) {
			if(err) {
				...
				console.log("Successfully saved " + slug + " list to cache.");
			}
		});
		callback(posts);
	});
